...
namespace Projeto02_WCFPagamentos
{
	public class ServicePagamentos : IServicePagamentos
	{
		public int EfetuarPagamento(PagamentoContract pagamento)
		{
			 ...
			 * 2- Pedido já pago
			 * 3- Limite Indiponível no cartão
			 */
			...
			using (var context = new DB_PAGAMENTOSEntities())
			{
				...
				var cartao = context.Cartoes.FirstOrDefault
					(c => c.NumeroCartao == pagamento.NumeroCartao);
				if (cartao == null)
				{
					return 1;
				}

				//Verficação do pedido
				...
				var pagamentosRealizados = context.Pagamentos.Where
